...
dns_googledomains_add() {
  ...
  if _dns_googledomains_post "$zone" "rotateChallenges" "{\"accessToken\":\"$GOOGLEDOMAINS_ACCESS_TOKEN\",\"recordsToAdd\":[{\"fqdn\":\"$fulldomain\",\"digest\":\"$txtvalue\"}],\"keepExpiredRecords\":true}"; then
...
dns_googledomains_rm() {
  ...
  if _dns_googledomains_post "$zone" "rotateChallenges" "{\"accessToken\":\"$GOOGLEDOMAINS_ACCESS_TOKEN\",\"recordsToRemove\":[{\"fqdn\":\"$fulldomain\",\"digest\":\"$txtvalue\"}],\"keepExpiredRecords\":true}"; then
    if _contains "$response" "$txtvalue"; then
...
_dns_googledomains_get_zone() {
  ...
  if [ "$GOOGLEDOMAINS_ZONE" ]; then
    if ! _dns_googledomains_get "$GOOGLEDOMAINS_ZONE"; then
      return 1
    fi
  ...
  while true; do
    ...
    if _dns_googledomains_get "$curr"; then
      echo "$curr"
      return 0
    fi
...
_dns_googledomains_get() {
  zone=$1

  response="$(_get "$GOOGLEDOMAINS_API/$zone")"
...
_dns_googledomains_post() {
  ...
  response="$(_post "$data" "$GOOGLEDOMAINS_API/$zone:$method")"
